...
if uploaded_file is not None:
    try:
        ...
        # 2. การวิเคราะห์สถิติเบื้องต้น (ปรับปรุงให้อ่านง่าย)
        st.subheader("📈 สถิติเชิงพรรณนา (Descriptive Statistics)")
        
        desc_df = df.describe()
        if not desc_df.empty:
            # เปลี่ยนชื่อแถวให้เป็นภาษาไทยให้อ่านง่าย
            index_mapping = {
        ...
        # ตรวจสอบประเภทข้อมูล (ดักจับทั้งแบบข้อความและหมวดหมู่)
        if str(df[selected_col].dtype) in ['object', 'bool', 'category', 'string']:
            ...
            # --- เพิ่มตัวเลขพร้อมลูกน้ำ (,) บนกราฟแท่ง ---
            ...
            # แสดงตารางสัดส่วนแบบทศนิยม 2 ตำแหน่ง พร้อมใส่เครื่องหมาย %
            st.dataframe(probabilities.rename("ความน่าจะเป็น (%)").to_frame().style.format("{:,.2f}%"))
            ...
            # --- เพิ่มตัวเลขพร้อมลูกน้ำ (,) บนกราฟ Histogram ---
            max_height = max([p.get_height() for p in ax.patches]) if ax.patches else 1
            for p in ax.patches:
                height = p.get_height()
                if height > 0: # ป้องกันการเขียนเลข 0 รกกราฟ
